						...
						success : function (aEntitySet) {
							...
							return this.waitFor({
								...
								check: function (oList) {
									...
									var sObjectID = oList.getBindingContext().getProperty("custid");

									var iLength = aEntitySet.filter(function (oLineItem) {
										return oLineItem.custid === sObjectID;
									}).length;
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							return this.waitFor({
								id : "lineItemsHeader",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name: "text", value: "<HisOrdersPlural> (" + iNumberOfItems + ")"}),
								success : function () {
